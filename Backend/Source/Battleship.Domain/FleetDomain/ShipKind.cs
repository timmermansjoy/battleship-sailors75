...
namespace Battleship.Domain.FleetDomain
{
    ...
    public class ShipKind
    {
        ...
        public GridCoordinate[] GenerateRandomSegmentCoordinates(int gridSize, bool allowDeformedShips = false)
        {
            ...
            GridCoordinate[] coords = new GridCoordinate[this.Size];

            /* Get orientation */
            Direction dir = Direction.CreateRandomly(allowDeformedShips);

            if (dir == Direction.West)
            {
                row = rand.Next(0, gridSize);
                col = rand.Next(this.Size - 1, gridSize);
            }
            else if (dir == Direction.East)
            {
                row = rand.Next(0, gridSize);
                col = rand.Next(0, gridSize - this.Size);
            }
            else if (dir == Direction.South)
            {
                row = rand.Next(0, gridSize - this.Size);
                col = rand.Next(0, gridSize);
            }
            else if (dir == Direction.North)
            {
                row = rand.Next(this.Size-1, gridSize);
                col = rand.Next(0, gridSize);
            }
            ...
            coords[0] = new GridCoordinate(row, col);
            for (int index = 1; index < this.Size; index++)
            {
                coords[index] = coords[index - 1].GetNeighbor(dir);
            }
            return coords;
